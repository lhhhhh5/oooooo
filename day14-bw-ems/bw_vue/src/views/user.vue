...
export default {
    ...
    methods: {
        loadDataList: function() {
            ...
            that.$http('user/selectUserByCondition', 'POST', data, true, function(resp) {
                let page = resp.page;
                let list = page.list;
                for (let one of list) {
                    if (one.status == 1) {
        ...
        loadRoleList: function() {
            let that = this;
            that.$http('role/selectAllRole', 'GET', null, true, function(resp) {
                that.roleList = resp.list;
            });
        },
        loadDeptList: function() {
            let that = this;
            that.$http('dept/selectAllDept', 'GET', null, true, function(resp) {
